...
def sample_gt(gt, percentage0, percentage1, mode='random'):
    ...
    if mode == 'random':
        ...
        for l in np.unique(gt):
            if l == 0:
                x, y = np.nonzero(gt == l)
                indices = np.random.choice(len(x), int(len(x) * percentage0),
                                       replace=False)
                x, y = x[indices], y[indices]
                mask[x, y] = True
            elif l == 1:
                x, y = np.nonzero(gt == l)
                indices = np.random.choice(len(x), int(len(x) * percentage1),
                                           replace=False)
                x, y = x[indices], y[indices]
                mask[x, y] = True
        ...
        for c in np.unique(gt):
            ...
            for x in range(gt.shape[0]):
                ...
                try:
                    ...
                    if ratio > 0.9 * percentage0 and ratio < 1.1 * percentage0:
                        break
                except ZeroDivisionError:
                    continue
            mask[:x, :] = 0
